...
class GetLogger:
    ...
    def __init__(self, log_name="app", logs_dir=None, logs_level=logging.INFO, logs_backup=15, console_out=True):
        self.logs_dir = logs_dir  # 日志路径
        self.log_name = log_name  # 日志名称
        self.logs_level = logs_level  # 日志级别
        self.logs_backup = logs_backup
        self.console_out = console_out
        # 日志的输出格式
        self.log_formatter = logging.Formatter('{"time":"%(asctime)s","file":"%(filename)s:%(lineno)d","level":"%(levelname)s","msg":%(message)s}')

        if logs_dir is None:
            sep = os.sep  # 自动匹配win,mac,linux 下的路径分隔符
            self.logs_dir = os.path.abspath(os.path.join(__file__, f"..{sep}..{sep}logs{sep}"))  # 设置日志保存路径
    ...
    def getLogger(self):
        ...
        # 实例化root日志对象
        logLogger = logging.getLogger('root')

        # 设置日志的输出级别
        logLogger.setLevel(self.logs_level)

        if not logLogger.handlers:  # 避免重复日志
            ...
            if self.console_out:
                ...
                logLogger.addHandler(console_handler)
            ...
            file_handler = ConcurrentTimedRotatingFileHandler(
                ...
                backupCount=self.logs_backup,  # 最多存放日志的数量
            ...
            logLogger.addHandler(file_handler)
